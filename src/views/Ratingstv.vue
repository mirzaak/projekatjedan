    ...
    <div class="top" v-if="ratedtv">
        <div class="left" v-if=" rated">
            ...
            <div>
                ...
                <router-link :to="{ name: 'Ratingstv' }"><a href="#">TV {{ ratedtv.total_results}}</a></router-link>
            </div>
        </div>
        <div class="right">
            <p>Filtered by:</p>
...
.then(data => console.log(this.account))
    ...
    fetch('https://api.themoviedb.org/3/account/{account_id}/rated/tv?api_key=0b5e8ce7494ae54d6c643adf4db40da7&session_id=f95caa0f4752ba79268ad7a023c023a70514310b')
    .then(response => response.json())
    .then(data => this.ratedtv = data)
    .then(data => console.log(data,'rated'))
